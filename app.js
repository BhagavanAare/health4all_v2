...
MongoClient.connect(url, function(err, db) {
    if (err)
        ...
        coll1 = DB.collection('transactions')
    }
});
    ...
    if (sess.number == config.get('app.admin')) {
        ...
        coll.findOne({ number: req.body.number }, function(err, results) {
            if (err) throw err;
                ...
                result = coll1.insertOne({ 'source': 'BANK', 'destination': req.body.number, 'amount': bal, 'status': 'initial' }, function(err, res1) {
    ...
    if (sess.number) {
        ...
        coll.findOne({ number: sess.number }, function(err, results) {
            if (err) throw err;
                ...
                amount1 = results.money
                result = coll1.insertOne({ 'source': sess.number, 'destination': 'ATM', 'amount': bal, 'status': 'initial' }, function(err, res1) {
                    inserted_id = res1.insertedId
        ...
        coll.findOne({ number: sess.number }, function(err, results) {
            ...
            coll.findOne({ number: destination }, function(err, results1) {
                ...
                if (results1 && (sess.number != destination) && (destination != config.get('app.admin')) && sta != 0 && sta1 != 0) {
                    coll1.insertOne({ 'source': source, 'destination': destination, 'amount': amount1, 'status': 'initial' }, function(err, results2) {
                        ...
                        coll.findOne({ number: sess.number }, function(err, results3) {
                            ...
                            if (first == second) {
                                coll1.updateOne({ '_id': results2.insertedId }, { $set: { 'status': 'transaction fails' } }, function(err, results4) {
                                    res.render('trans', { status: 'last transaction fails', number: sess.number, val: false })
                                ...
                                coll.update({ number: destination }, { $inc: { 'money': amount1 } }, function(err, results5) {
                                    coll1.updateOne({ '_id': results2.insertedId }, { $set: { 'status': 'transaction sucess' } }, function(err, reuslts6) {
                                        res.render('trans', { status: 'last transaction success', number: sess.number, val: false })
                                    })
                                })
                            }
